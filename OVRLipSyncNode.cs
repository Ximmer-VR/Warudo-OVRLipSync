...
public class OVRLipSyncNode : Node
{
    ...
    public class VisemeData : StructuredData, ICollapsibleStructuredData
    {
        ...
        [Label("Viseme")]
        [Disabled]
        public OVRLipSync.Viseme _viseme;
    ...
    // Automap dictionary
    readonly Dictionary<OVRLipSync.Viseme, List<string>> _automap = new Dictionary<OVRLipSync.Viseme, List<string>>()
    {
        { OVRLipSync.Viseme.SIL, new List<string>() { "vrc/sil", "vrc.v_sil", "sil", "" } },
        { OVRLipSync.Viseme.PP,  new List<string>() { "vrc/pp",  "vrc.v_pp",  "pp",  "" } },
        { OVRLipSync.Viseme.FF,  new List<string>() { "vrc/ff",  "vrc.v_ff",  "ff",  "" } },
        { OVRLipSync.Viseme.TH,  new List<string>() { "vrc/th",  "vrc.v_th",  "th",  "" } },
        { OVRLipSync.Viseme.DD,  new List<string>() { "vrc/dd",  "vrc.v_dd",  "dd",  "" } },
        { OVRLipSync.Viseme.KK,  new List<string>() { "vrc/kk",  "vrc.v_kk",  "kk",  "" } },
        { OVRLipSync.Viseme.CH,  new List<string>() { "vrc/ch",  "vrc.v_ch",  "ch",  "" } },
        { OVRLipSync.Viseme.SS,  new List<string>() { "vrc/ss",  "vrc.v_ss",  "ss",  "" } },
        { OVRLipSync.Viseme.NN,  new List<string>() { "vrc/nn",  "vrc.v_nn",  "nn",  "" } },
        { OVRLipSync.Viseme.RR,  new List<string>() { "vrc/rr",  "vrc.v_rr",  "rr",  "" } },
        { OVRLipSync.Viseme.AA,  new List<string>() { "vrc/aa",  "vrc.v_aa",  "aa",  "a" } },
        { OVRLipSync.Viseme.E ,  new List<string>() { "vrc/e",   "vrc.v_e" ,  "e",   "e" } },
        { OVRLipSync.Viseme.IH,  new List<string>() { "vrc/ih",  "vrc.v_ih",  "ih",  "i" } },
        { OVRLipSync.Viseme.OH,  new List<string>() { "vrc/oh",  "vrc.v_oh",  "oh",  "o" } },
        { OVRLipSync.Viseme.OU,  new List<string>() { "vrc/ou",  "vrc.v_ou",  "ou",  "u" } },
    };
    ...
    public Continuation AutoMapVisemes()
    {
        ...
        foreach (KeyValuePair<OVRLipSync.Viseme, List<string>> item in _automap)
        {
            string best = "";
            int bestDist = 200;
            bool found = false;
    ...
    protected override void OnCreate()
    {
        ...
        if(_visemes == null || _visemes.Length == 0)
        {
            ...
            for (int i = 0; i < _visemes.Length; i++)
            {
                ...
                _visemes[i]._viseme = (OVRLipSync.Viseme)i;
            }
        }
        ...
        if (OVRLipSync.CreateContext(ref _context, provider, 0, enableAcceleration) != OVRLipSync.Result.Success)
        {
            _context = 0;
        ...
        {
            OVRLipSync.Result result = OVRLipSync.SendSignal(_context, OVRLipSync.Signals.VisemeSmoothing, _smoothing, 0);
        ...
        {
            OVRLipSync.DestroyContext(_context);
            if (OVRLipSync.CreateContext(ref _context, provider, 0, enableAcceleration) != OVRLipSync.Result.Success)
            {
                _context = 0;
                Context.Service.Toast(Warudo.Core.Server.ToastSeverity.Error, nodeTitle, "Error initializing OVRLipSync Context");
    ...
    {
        _audioBuffer[_audioBufferIndex++] = (e.Left + e.Right) * _gain;
        if(_audioBufferIndex >= _audioBuffer.Length)
        {
            _audioBufferIndex = 0;

            if (_context == 0 || OVRLipSync.IsInitialized() != OVRLipSync.Result.Success)
            {
                return;
            }
            OVRLipSync.ProcessFrame(_context, _audioBuffer, frame, false);
        }
    }
    ...
    public override void OnUpdate()
    {
        ...
        {
            _debugOutput = "";
            for (int i = 0; i < Enum.GetNames(typeof(OVRLipSync.Viseme)).Length; i++)
            {
                _debugOutput += Enum.GetName(typeof(OVRLipSync.Viseme), i) + ": " + ((int)(frame.Visemes[i] * 100)).ToString() + "%\n\n";
            }
